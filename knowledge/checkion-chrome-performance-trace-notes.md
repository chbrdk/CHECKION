...
- **`ScannedPagesTable`:** Kein `useVirtualizer` mehr — alle sichtbaren Slim-Page-Zeilen einer Seite liegen im DOM (typisch ≤ `DOMAIN_SLIM_PAGES_PAGE_SIZE`). **`VirtualChipList`:** immer flex-wrap + `overflow: auto`, kein verschachteltes Virtual-Scroll.
- **Weitere statische Imports:** `ScannedPagesTable`, `DomainIssuesMasterDetail`, `DomainGraph`, `DomainToolsCard`, `JourneyFlowchart` direkt aus den jeweiligen Domain-Sektionen (kein dynamisches Nachladen dieser Teile).
