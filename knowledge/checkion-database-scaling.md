...
| **Connection Pool** | Wartezeiten unter Last | `DATABASE_POOL_MAX` → `lib/db/pool-config.ts` + Monitoring |
...
2. **Projektionen:** Hot-Pfade in `lib/db/` auf schmale `select({ … })` umstellen (siehe Merge db-pool PR).
3. **Pool-Konfiguration:** `DATABASE_POOL_MAX` (Standard 10, Grenzen 1–100) — `lib/db/pool-config.ts`, `ENV_DATABASE_POOL_MAX`.
4. **Keyset (optional):** `GET .../slim-pages` und `.../seo-pages` unterstützen `after=<base64url-json>` (gleiche `sort`/`dir`); Antwort kann `nextCursor` liefern. UI nutzt Keyset für Vor/Zurück bei DB-Quelle; `offset` bleibt für Kompatibilität.
