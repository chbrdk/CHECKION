...
**Wichtig:** Für `NEXT_PUBLIC_APP_BASE_URL` und `BASE_PATH` ist ein **Pfad** wie `/checkion` der saubere Zielwert. Falls aus Versehen eine vollständige URL gesetzt wird, reduziert CHECKION sie automatisch auf ihren Path-Anteil, damit der Produktionsbuild nicht an `next.config.mjs` scheitert.
