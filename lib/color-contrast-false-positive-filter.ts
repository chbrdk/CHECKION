...
const BROWSER_VERIFY_FN = function (
    ...
    function getPseudoBackground(el: Element, pseudo: '::before' | '::after') {
        const style = getComputedStyle(el, pseudo);
        // Pseudo-element must exist (content not 'none' or empty)
        if (!style.content || style.content === 'none') return null;

        const bg = parseRgba(style.backgroundColor);
        if (!bg || bg.a === 0) return null;

        // Common pattern: positioned pseudo with background acts as overlay
    ...
    function resolveBackground(el: Element) {
        ...
        while (node && node.tagName !== 'body') {
            // Check ::before (often used as background overlay with z-index: -1)
            const beforeBg = getPseudoBackground(node, '::before');
            if (beforeBg) stack.push(beforeBg);

            const bg = parseRgba(getComputedStyle(node).backgroundColor);
            if (bg && bg.a > 0) stack.push(bg);

            // Check ::after as well (less common but possible)
            const afterBg = getPseudoBackground(node, '::after');
            if (afterBg) stack.push(afterBg);
...
function messageSaysPass(issue: Issue): boolean {
    ...
    const large = parsed.fontSizePx != null ? isLargeText(parsed.fontSizePx, parsed.fontWeight ?? 400, false) : false;
    const required = enhanced ? (large ? 4.5 : 7) : requiredAaContrastRatio(large);
...
export async function filterContrastFalsePositives(page: Page, issues: Issue[]): Promise<ContrastFilterResult> {
    ...
    try {
        // Forward browser console.log to server terminal for debugging
        const consoleHandler = (msg: import('puppeteer').ConsoleMessage) => {
        ...
        // Remove listener after evaluate to avoid duplicate logs
