    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0.5 }}>
            ...
            >
                ...
                >
                    <MsqdxTypography variant="h6" sx={{ fontWeight: 700, color }}>
                        {Math.round(score)}
                    </MsqdxTypography>
                </Box>
            </Box>
    ...
    return (
        <Box sx={{ width: '100%', minWidth: 120 }}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.25 }}>
                ...
                <MsqdxTypography variant="caption" sx={{ fontWeight: 600, color }}>
                    {value} / {total}
                </MsqdxTypography>
            </Box>
...
function MetricBox({
    ...
    return (
        ...
        >
            <MsqdxTypography variant="caption" sx={{ color: STATUS.neutral, display: 'block' }}>
                {label}
            </MsqdxTypography>
            <MsqdxTypography variant="body1" sx={{ fontWeight: 700, color: statusColor || 'inherit' }}>
                {value}
                {unit ?? ''}
            </MsqdxTypography>
        </Box>
...
export default function ShareLandingPage() {
            ...
            return fetch(apiShareToken(tokenStr), {
                ...
                .then((res) => {
                    if (res.status === 403) {
                        ...
                        return clone
                            .json()
                            .then((b) => (b.requiresPassword ? Promise.reject(new Error('PASSWORD_REQUIRED')) : res));
                    }
                    if (!res.ok)
                        throw new Error(res.status === 404 ? t('share.linkInvalidOrExpired') : t('share.loadError'));
                    return res.json();
                })
                .then((data) => {
                    setPayload(data);
    ...
    useEffect(() => {
        ...
        if (!tokenStr) return;
        const stored =
            typeof window !== 'undefined' ? sessionStorage.getItem(SHARE_ACCESS_STORAGE_KEY + '_' + tokenStr) : null;
        if (stored) {
                ...
                .then((data) => {
                    if (data.accessToken) {
                        ...
                        if (typeof window !== 'undefined')
                            sessionStorage.setItem(SHARE_ACCESS_STORAGE_KEY + '_' + tokenStr, data.accessToken);
    ...
    if (!token) {
        return (
            <Box sx={{ p: 4, maxWidth: 600, mx: 'auto', textAlign: 'center', bgcolor: '#fff', minHeight: '100vh' }}>
                <MsqdxTypography variant="h6" sx={{ color: MSQDX_STATUS.error.base }}>
                    {t('share.linkInvalidOrExpired')}
                </MsqdxTypography>
            </Box>
        );
    }
    ...
    const authToken =
        accessToken ??
        (typeof window !== 'undefined' ? sessionStorage.getItem(SHARE_ACCESS_STORAGE_KEY + '_' + token) : null);

    return (
        <Box
            sx={{ p: 'var(--msqdx-spacing-md)', maxWidth: 900, mx: 'auto', bgcolor: '#fff', minHeight: '100vh' }}
        ...
        >
            ...
            {payload.type === 'domain' && (
                <ShareDomainContent data={payload.data} token={token} accessToken={authToken} />
            )}
            {payload.type === 'single' && (
                <ShareSingleContent data={payload.data} shareToken={token} accessToken={authToken} />
            )}
...
function ShareDomainContent({
    data,
    token,
    accessToken,
}: {
    data: DomainSummaryResponse;
    token: string;
    ...
    const paginatedPages = pages.slice(
        (listPage - 1) * SHARE_DOMAIN_PAGES_PAGE_SIZE,
        listPage * SHARE_DOMAIN_PAGES_PAGE_SIZE,
    );
        ...
        <>
            ...
            >
                <MsqdxTypography variant="h5" sx={{ fontWeight: 700, mb: 1 }}>
                    {t('share.deepScanTitle')}: {data.domain}
                </MsqdxTypography>
                <MsqdxTypography variant="body2" sx={{ color: STATUS.neutral, mb: 2 }}>
                    <span suppressHydrationWarning>{new Date(data.timestamp).toISOString().slice(0, 10)}</span> ·{' '}
                    {data.totalPages} {t('share.pages')}
                </MsqdxTypography>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap' }}>
                    ...
                    {agg?.issues && (
                        <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                            <MsqdxChip
                                label={`${agg.issues.stats.errors} ${t('share.errors')}`}
                                size="small"
                                variant="outlined"
                                sx={{ bgcolor: alpha(STATUS.error, 0.12), color: STATUS.error }}
                            />
                            <MsqdxChip
                                label={`${agg.issues.stats.warnings} ${t('share.warnings')}`}
                                size="small"
                                variant="outlined"
                                sx={{ bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }}
                            />
                            <MsqdxChip
                                label={`${agg.issues.stats.notices} ${t('share.notices')}`}
                                size="small"
                                variant="outlined"
            ...
            {agg?.performance && (
                ...
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1.5 }}>
                        {t('share.performanceAvg')}
                    </MsqdxTypography>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                        <MetricBox
                            label="Ø TTFB"
                            value={agg.performance.avgTtfb}
                            unit=" ms"
                            statusColor={getPerformanceColor(agg.performance.avgTtfb, 'ttfb')}
                        />
                        <MetricBox
                            label="Ø FCP"
                            value={agg.performance.avgFcp}
                            unit=" ms"
                            statusColor={getPerformanceColor(agg.performance.avgFcp, 'fcp')}
                        />
                        <MetricBox
                            label="Ø LCP"
                            value={agg.performance.avgLcp}
                            unit=" ms"
                            statusColor={getPerformanceColor(agg.performance.avgLcp, 'lcp')}
                        />
                        <MetricBox
                            label="Ø DOM Load"
                            value={agg.performance.avgDomLoad}
                            unit=" ms"
                            statusColor={getPerformanceColor(agg.performance.avgDomLoad, 'dom')}
                        />
                        <Box sx={{ alignSelf: 'flex-end', py: 1 }}>
                            <MsqdxTypography variant="caption" sx={{ color: STATUS.neutral }}>
                                {agg.performance.pageCount} {t('share.pages')}
                            </MsqdxTypography>
                        </Box>
                    </Box>
                </MsqdxCard>
            ...
            {agg?.eco && (
                ...
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1.5 }}>
                        Ökobilanz (zusammengefasst)
                    </MsqdxTypography>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, alignItems: 'flex-start' }}>
                        <MetricBox
                            label="Ø CO₂"
                            value={agg.eco.avgCo2}
                            unit=" g"
                            statusColor={getEcoCo2Color(agg.eco.avgCo2)}
                        />
                        <MetricBox
                            label="Gesamtgewicht"
                            value={(agg.eco.totalPageWeight / 1024).toFixed(0)}
                            unit=" KB"
                        />
                        {Object.keys(agg.eco.gradeDistribution).length > 0 && (
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, alignItems: 'center' }}>
                                <MsqdxTypography variant="caption" sx={{ color: STATUS.neutral, mr: 0.5 }}>
                                    Grade:
                                </MsqdxTypography>
                                {Object.entries(agg.eco.gradeDistribution)
                                    ...
                                    .map(([grade, count]) => (
                                        <MsqdxChip
                                            ...
                                            sx={{
                                                fontSize: '0.7rem',
                                                bgcolor: alpha(getEcoGradeColor(grade), 0.15),
                                                color: getEcoGradeColor(grade),
                                            }}
                                        />
                                    ))}
                            </Box>
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, alignItems: 'flex-start' }}>
                        ...
                        <MetricBox
                            ...
                            value={agg.ux.cls}
                            statusColor={
                                agg.ux.cls <= 0.1 ? STATUS.good : agg.ux.cls <= 0.25 ? STATUS.warning : STATUS.error
                            }
                        />
                        <MetricBox
                            label="Kaputte Links"
                            value={agg.ux.brokenLinks.length}
                            statusColor={agg.ux.brokenLinks.length > 0 ? STATUS.error : undefined}
                        />
                        <MetricBox
                            label="Seiten mit Console-Errors"
                            value={agg.ux.consoleErrorsByPage.length}
                            statusColor={agg.ux.consoleErrorsByPage.length > 0 ? STATUS.error : undefined}
                        />
                        {agg.ux.tapTargets.detailsByPage.length > 0 && (
                            <MetricBox
                                label="Touch-Target-Probleme (Seiten)"
                                value={agg.ux.tapTargets.detailsByPage.length}
                ...
                >
                    ...
                    <Box
                        sx={{
                            ...
                            gap: 2,
                            mb: agg.seo.crossPageKeywords.length > 0 ? 1.5 : 0,
                        }}
                    >
                        <MetricBar
                            label="Mit Titel"
                            value={agg.seo.withTitle}
                            total={agg.seo.totalPages}
                            color={getRatioColor(agg.seo.withTitle / agg.seo.totalPages)}
                        />
                        <MetricBar
                            label="Mit Meta-Description"
                            value={agg.seo.withMetaDescription}
                            total={agg.seo.totalPages}
                            color={getRatioColor(agg.seo.withMetaDescription / agg.seo.totalPages)}
                        />
                        <MetricBar
                            label="Mit H1"
                            value={agg.seo.withH1}
                            total={agg.seo.totalPages}
                            color={getRatioColor(agg.seo.withH1 / agg.seo.totalPages)}
                        />
                        {agg.seo.missingMetaDescriptionUrls.length > 0 && (
                            <Box sx={{ gridColumn: '1 / -1' }}>
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={`Ohne Meta-Description: ${agg.seo.missingMetaDescriptionUrls.length} Seiten`}
                                    sx={{ bgcolor: alpha(STATUS.error, 0.12), color: STATUS.error }}
                                />
                            </Box>
                        )}
            ...
            {agg?.links && (
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                        ...
                        <MetricBox
                            label="Kaputte (unique)"
                            value={agg.links.uniqueBrokenUrls}
                            statusColor={agg.links.uniqueBrokenUrls > 0 ? STATUS.error : undefined}
                        />
                    </Box>
                    {agg.links.uniqueBrokenUrls > 0 && agg.links.totalLinks > 0 && (
                        <Box sx={{ mt: 1.5 }}>
                            <MetricBar
                                label="Intakte vs. kaputte Links"
                                value={agg.links.totalLinks - agg.links.uniqueBrokenUrls}
                                total={agg.links.totalLinks}
                                color={getRatioColor(
                                    (agg.links.totalLinks - agg.links.uniqueBrokenUrls) / agg.links.totalLinks,
                                )}
                            />
                        </Box>
                    )}
            ...
            {agg?.infra?.technicalCounts && (
                ...
                >
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
                        <MetricBar
                            label="Manifest"
                            value={agg.infra.technicalCounts.withManifest}
                            total={agg.infra.technicalCounts.pageCount}
                            color={getRatioColor(
                                agg.infra.technicalCounts.withManifest / agg.infra.technicalCounts.pageCount,
                            )}
                        />
                        <MetricBar
                            label="Theme-Color"
                            value={agg.infra.technicalCounts.withThemeColor}
                            total={agg.infra.technicalCounts.pageCount}
                            color={getRatioColor(
                                agg.infra.technicalCounts.withThemeColor / agg.infra.technicalCounts.pageCount,
                            )}
                        />
                        <MetricBar
                            label="Apple Touch Icon"
                            value={agg.infra.technicalCounts.withAppleTouchIcon}
                            total={agg.infra.technicalCounts.pageCount}
                            color={getRatioColor(
                                agg.infra.technicalCounts.withAppleTouchIcon / agg.infra.technicalCounts.pageCount,
                            )}
                        />
                        <MetricBar
                            label="Service Worker"
                            value={agg.infra.technicalCounts.withServiceWorker}
                            total={agg.infra.technicalCounts.pageCount}
                            color={getRatioColor(
                                agg.infra.technicalCounts.withServiceWorker / agg.infra.technicalCounts.pageCount,
                            )}
                        />
                    </Box>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mt: 1, alignItems: 'center' }}>
                        {agg.infra.technicalCounts.pagesWithRedirects > 0 && (
                            <MsqdxChip
                                size="small"
                                variant="outlined"
                                label={`Redirects ${agg.infra.technicalCounts.pagesWithRedirects} Seiten`}
                                sx={{ bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }}
                            />
                        )}
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
                        {agg.infra.privacy.totalPages > 0 && (
                            <>
                                <MetricBar
                                    label="Datenschutz"
                                    value={agg.infra.privacy.withPolicy}
                                    total={agg.infra.privacy.totalPages}
                                    color={getRatioColor(agg.infra.privacy.withPolicy / agg.infra.privacy.totalPages)}
                                />
                                <MetricBar
                                    label="Cookie-Hinweis"
                                    value={agg.infra.privacy.withCookieBanner}
                                    total={agg.infra.privacy.totalPages}
                                    color={getRatioColor(
                                        agg.infra.privacy.withCookieBanner / agg.infra.privacy.totalPages,
                                    )}
                                />
                                <MetricBar
                                    label="AGB"
                                    value={agg.infra.privacy.withTerms}
                                    total={agg.infra.privacy.totalPages}
                                    color={getRatioColor(agg.infra.privacy.withTerms / agg.infra.privacy.totalPages)}
                                />
                            </>
                        )}
                        {agg.infra.security.totalPages > 0 && (
                            <>
                                <MetricBar
                                    label="CSP"
                                    value={agg.infra.security.withCsp}
                                    total={agg.infra.security.totalPages}
                                    color={getRatioColor(agg.infra.security.withCsp / agg.infra.security.totalPages)}
                                />
                                <MetricBar
                                    label="X-Frame-Options"
                                    value={agg.infra.security.withXFrame}
                                    total={agg.infra.security.totalPages}
                                    color={getRatioColor(agg.infra.security.withXFrame / agg.infra.security.totalPages)}
                                />
                            </>
                        )}
                    </Box>
            ...
            {agg?.structure && (
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                        <MetricBox
                            label="Seiten mit mehreren H1"
                            value={agg.structure.pagesWithMultipleH1.length}
                            statusColor={agg.structure.pagesWithMultipleH1.length > 0 ? STATUS.error : undefined}
                        />
                        <MetricBox
                            label="Seiten mit Level-Sprüngen"
                            value={agg.structure.pagesWithSkippedLevels.length}
                            statusColor={agg.structure.pagesWithSkippedLevels.length > 0 ? STATUS.warning : undefined}
                        />
                        <MetricBox
                            label="Gute Struktur"
                            value={agg.structure.pagesWithGoodStructure.length}
                            statusColor={agg.structure.pagesWithGoodStructure.length > 0 ? STATUS.good : undefined}
                        />
                    </Box>
                </MsqdxCard>
            )}
            ...
            {data.eeat && (
                ...
                >
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>
                        <MetricBar
                            label="Impressum"
                            value={data.eeat.trust.pagesWithImpressum}
                            total={data.eeat.trust.totalPages}
                            color={getRatioColor(data.eeat.trust.pagesWithImpressum / data.eeat.trust.totalPages)}
                        />
                        <MetricBar
                            label="Kontakt"
                            value={data.eeat.trust.pagesWithContact}
                            total={data.eeat.trust.totalPages}
                            color={getRatioColor(data.eeat.trust.pagesWithContact / data.eeat.trust.totalPages)}
                        />
                        <MetricBar
                            label="Datenschutz"
                            value={data.eeat.trust.pagesWithPrivacy}
                            total={data.eeat.trust.totalPages}
                            color={getRatioColor(data.eeat.trust.pagesWithPrivacy / data.eeat.trust.totalPages)}
                        />
                        <MetricBar
                            label="Über uns"
                            value={data.eeat.experience.pagesWithAbout}
                            total={data.eeat.experience.totalPages}
                            color={getRatioColor(data.eeat.experience.pagesWithAbout / data.eeat.experience.totalPages)}
                        />
                        <MetricBar
                            label="Team"
                            value={data.eeat.experience.pagesWithTeam}
                            total={data.eeat.experience.totalPages}
                            color={getRatioColor(data.eeat.experience.pagesWithTeam / data.eeat.experience.totalPages)}
                        />
                        <MetricBar
                            label="Autor-Bio"
                            value={data.eeat.expertise.pagesWithAuthorBio}
                            total={data.eeat.expertise.totalPages}
                            color={getRatioColor(
                                data.eeat.expertise.pagesWithAuthorBio / data.eeat.expertise.totalPages,
                            )}
                        />
                    </Box>
                    <MsqdxTypography variant="caption" sx={{ color: STATUS.neutral, mt: 1 }}>
                        Ø Zitate/Seite: {data.eeat.expertise.avgCitationsPerPage.toFixed(1)}
                    </MsqdxTypography>
                </MsqdxCard>
            )}
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 3, alignItems: 'flex-start' }}>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            <MetricBar
                                label="Seiten mit LLMs.txt"
                                value={agg.generative.withLlmsTxt}
                                total={agg.generative.pageCount}
                                color={getRatioColor(agg.generative.withLlmsTxt / agg.generative.pageCount)}
                            />
                            <MetricBar
                                label="Robots erlauben AI"
                                value={agg.generative.withRobotsAllowingAi}
                                total={agg.generative.pageCount}
                                color={getRatioColor(agg.generative.withRobotsAllowingAi / agg.generative.pageCount)}
                            />
                        </Box>
                    </Box>
                </MsqdxCard>
            ...
            {data.systemicIssues && data.systemicIssues.length > 0 && (
                <MsqdxCard
                    ...
                    sx={{
                        ...
                        mb: 2,
                        borderRadius: 2,
                        border: `1px solid ${alpha(STATUS.error, 0.3)}`,
                        bgcolor: alpha(STATUS.error, 0.04),
                    }}
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1, color: STATUS.error }}>
                        Systemische Issues
                    </MsqdxTypography>
                    {data.systemicIssues.map((issue, idx) => (
                        <Box key={idx} sx={{ mb: 1, display: 'flex', alignItems: 'center', gap: 1 }}>
                            ...
                            <MsqdxTypography variant="body2" sx={{ color: STATUS.error }}>
                                {issue.title} — {issue.count} Seiten
                            </MsqdxTypography>
                        </Box>
                    ))}
                </MsqdxCard>
            ...
            >
                <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1.5 }}>
                    {t('share.scannedPages', { count: String(pages.length) })}
                </MsqdxTypography>
                        ...
                        <li key={p.id}>
                            ...
                            >
                                <Box sx={{ flex: '1 1 200px', minWidth: 0 }}>
                                    <MsqdxTypography variant="body2" sx={{ wordBreak: 'break-all' }}>
                                        {p.url}
                                    </MsqdxTypography>
                                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mt: 0.5 }}>
                                        <MsqdxTypography
                                            variant="caption"
                                            sx={{ fontWeight: 600, color: getScoreColor(p.score) }}
                                        >
                                            Score {p.score}
                                        </MsqdxTypography>
                                        {p.stats.errors > 0 && (
                                            <MsqdxChip
                                                size="small"
                                                variant="outlined"
                                                label={`${p.stats.errors} E`}
                                                sx={{
                                                    height: 20,
                                                    fontSize: '0.7rem',
                                                    bgcolor: alpha(STATUS.error, 0.12),
                                                    color: STATUS.error,
                                                }}
                                            />
                                        )}
                                        {p.stats.warnings > 0 && (
                                            <MsqdxChip
                                                size="small"
                                                variant="outlined"
                                                label={`${p.stats.warnings} W`}
                                                sx={{
                                                    height: 20,
                                                    fontSize: '0.7rem',
                                                    bgcolor: alpha(STATUS.warning, 0.12),
                                        ...
                                        {p.stats.notices > 0 && (
                                            <MsqdxChip
                                                size="small"
                                                variant="outlined"
                                                label={`${p.stats.notices} N`}
                                                sx={{
                                                    height: 20,
                                                    fontSize: '0.7rem',
                                                    bgcolor: alpha(MSQDX_STATUS.info.base, 0.12),
                                ...
                                <Button
                                    ...
                                    sx={{ flexShrink: 0 }}
                                >
                                    {loadingDetail && loadingPageId === p.id
                                        ? t('share.loading')
                                        : t('share.showSingleScan')}
                                </Button>
                            </Box>
                        </li>
                    ))}
                    ...
                    >
                        ...
                        <Box sx={{ display: 'flex', gap: 0.5 }}>
                            ...
                            <Button
                                ...
                                onClick={() => setListPage((x) => Math.min(totalPages, x + 1))}
                            >
                                {t('share.next')}
                            </Button>
                        </Box>
            ...
            <Dialog
                open={!!selectedPageDetail || loadingDetail}
                onClose={() => !loadingDetail && setSelectedPageDetail(null)}
                maxWidth="md"
                fullWidth
                scroll="paper"
            ...
            >
                ...
                >
                    <MsqdxTypography variant="h6" sx={{ color: '#1a1a1a' }}>
                        {t('share.singleScanTitle')}
                    </MsqdxTypography>
                    <IconButton
                    ...
                    {selectedPageDetail && !loadingDetail && (
                        <Box
                            sx={{
                                ...
                                '& .MuiChip-root': { color: 'inherit' },
                            }}
                        >
                            <ShareSingleContent
                                data={selectedPageDetail}
                                shareToken={token}
    ...
    return (
        <>
            ...
            >
                ...
                <MsqdxTypography variant="body2" sx={{ color: STATUS.neutral, wordBreak: 'break-all' }}>
                    {data.url}
                </MsqdxTypography>
            </MsqdxCard>
            {pages.length > 0 && (
                        ...
                        >
                            ...
                            {page.technical?.generative && (
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={`GEO: ${page.technical.generative.score}`}
                                    sx={{ mt: 0.5 }}
                                />
                            )}
                            {page.geoFitnessScore != null && (
                                <MsqdxTypography variant="caption" sx={{ display: 'block', color: STATUS.neutral }}>
                                    Fitness: {page.geoFitnessScore}
                                </MsqdxTypography>
                            )}
                        </Box>
                    ))}
            ...
            {recommendations.length > 0 && (
                ...
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1.5 }}>
                        {t('geoEeat.recommendationsTitle')}
                    </MsqdxTypography>
            ...
            {comp?.metrics != null && comp.metrics.length > 0 && (
                ...
                >
                    {sourceByModel && Object.keys(sourceByModel).length > 0 && data.url && (
                        <CompetitivePositionDiagram competitiveByModel={sourceByModel} targetUrl={data.url} t={t} />
                    )}
                    {hasMultiModel && (
                    ...
                    {hasMultiModel && (
                        ...
                        >
                            {t('geoEeat.competitiveModelLabel', {
                                model: competitiveModelsFromSource[currentModelIndex] ?? '',
                            })}
                        </MsqdxTypography>
                    )}
                                    ...
                                    >
                                        <Box
                                            sx={{
                                                ...
                                                cursor: 'default',
                                                '&:hover': {
                                                    bgcolor: alpha(DOMAIN_COLORS[idx % DOMAIN_COLORS.length], 0.06),
                                                },
                                            }}
                                        >
                                                    ...
                                                    {[...run.citations]
                                                        ...
                                                        .map((c, cIdx) => (
                                                            ...
                                                            >
                                                                <MsqdxTypography
                                                                    variant="caption"
                                                                    sx={{ fontWeight: 600, color: textTertiary }}
                                                                >
...
function ShareJourneyContent({ data, accessToken }: { data: ShareJourneyData; accessToken?: string | null }) {
    ...
    const videoUrl = data.videoUrl
        ? accessToken
            ? `${data.videoUrl}?access=${encodeURIComponent(accessToken)}`
            : data.videoUrl
        : undefined;
        ...
        <>
            ...
            >
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
                    ...
                    {steps.length > 0 && (
                        <MsqdxTypography variant="caption" sx={{ color: STATUS.neutral }}>
                            {t('share.stepsCount', { count: String(steps.length) })}
                        </MsqdxTypography>
                    )}
                </Box>
            </MsqdxCard>
            ...
            {steps.length > 0 && (
                ...
                >
                    ...
                    <Box
                        component="ul"
                        sx={{ m: 0, pl: 0, listStyle: 'none', display: 'flex', flexDirection: 'column', gap: 1.5 }}
                    >
                        {steps.map((step, idx) => (
                            <Box
                                key={idx}
                                    ...
                                    <MsqdxChip
                                        ...
                                        label={
                                            step.action === 'navigate'
                                                ? t('share.stepLabelPage')
                                                : step.action === 'click'
                                                  ? t('share.stepLabelClick')
                                                  : step.action === 'done'
                                                    ? t('share.stepLabelDone')
                                                    : step.action
                                        }
                                        sx={{
                                            fontWeight: 600,
                                            ...
                                            ...(step.action === 'done'
                                                ? {
                                                      bgcolor: alpha(MSQDX_STATUS.success?.base ?? STATUS.good, 0.2),
                                                      color: MSQDX_STATUS.success?.base ?? STATUS.good,
                                                  }
                                                : step.action === 'navigate'
                                                  ? {
                                                        bgcolor: alpha(MSQDX_STATUS.info?.base ?? '#2196f3', 0.2),
                                                        color: MSQDX_STATUS.info?.base ?? '#2196f3',
                                                    }
                                                  : { bgcolor: alpha(STATUS.good, 0.2), color: STATUS.good }),
                                        }}
                                    />
                                    <MsqdxTypography variant="caption" sx={{ color: STATUS.neutral }}>
                                        {t('share.stepN', { n: String(idx + 1) })}
                                    </MsqdxTypography>
                                </Box>
...
function ShareSingleContent({
    ...
    const hasVisualAnalysis = shareToken
        ? true
        : Boolean(
              data.screenshot || data.saliencyHeatmap || data.pageIndex?.regions?.length || data.ux?.focusOrder?.length,
          );
    ...
    return (
        <>
            {hasVisualAnalysis && (
                ...
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1.5, color: '#1a1a1a' }}>
                        Visuelle Analyse
                    </MsqdxTypography>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, mb: 1.5 }}>
                        ...
                        {data.pageIndex?.regions?.length ? (
                            <Button
                                size="small"
                                variant={showRegions ? 'contained' : 'outlined'}
                    ...
                    {screenshotSrc && (
                        ...
                        >
                            {screenshotError ? (
                                <Box sx={{ py: 4, textAlign: 'center', color: '#666', fontSize: 14 }}>
                                    ...
                                    >
                                        ...
                                        {data.pageIndex && (
                                            <PageIndexRegionsOverlay
                                                regions={
                                                    Array.isArray(data.pageIndex?.regions) ? data.pageIndex.regions : []
                                                }
                                                screenshotWidth={screenshotDimensions.width}
                                                screenshotHeight={screenshotDimensions.height}
                    ...
                    {Array.isArray(data.pageIndex?.regions) && data.pageIndex.regions.length > 0 ? (
                        <Box sx={{ mt: 1.5, p: 1.5, bgcolor: '#f5f5f5', borderRadius: 1 }}>
                            <MsqdxTypography
                                variant="caption"
                                sx={{ fontWeight: 600, display: 'block', mb: 1, color: '#555' }}
                            >
                                Regionen — Hover zum Hervorheben
                            </MsqdxTypography>
                                ...
                                {data.pageIndex.regions.map((r) => (
                                    <Box
                                        ...
                                        sx={{
                                            ...
                                            fontSize: '0.75rem',
                                            bgcolor:
                                                hoveredRegionId === r.id
                                                    ? alpha(MSQDX_BRAND_PRIMARY.orange ?? '#ff6a3b', 0.25)
                                                    : 'transparent',
                                    ...
                                    >
                                        <Box component="span" sx={{ fontWeight: 600, color: '#666', mr: 0.5 }}>
                                            {r.tag}
                                        </Box>
                    ...
                    {data.pageIndex && (
                        <Box sx={{ mt: 1.5, color: '#1a1a1a' }}>
                            <PageIndexCard pageIndex={data.pageIndex} showSaliency={true} />
                        </Box>
                    )}
                </MsqdxCard>
            )}
            ...
            >
                <MsqdxTypography variant="h5" sx={{ fontWeight: 700, mb: 1, color: textPrimary }}>
                    Seiten-Scan
                </MsqdxTypography>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, flexWrap: 'wrap' }}>
                    ...
                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', alignItems: 'center' }}>
                        ...
                        />
                        <MsqdxChip
                            label={`${stats.notices} Notices`}
                            size="small"
                            variant="outlined"
                            sx={{ bgcolor: alpha(MSQDX_STATUS.info.base, 0.12), color: MSQDX_STATUS.info.base }}
                        />
                    </Box>
                    {data.ux?.score != null && <ScoreRing score={data.ux.score} size={52} label="UX-Score" />}
                </Box>
            </MsqdxCard>
            ...
            {perf && (
                ...
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1.5, color: textPrimary }}>
                        Performance
                    </MsqdxTypography>
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                        <MetricBox
                            ...
                            value={perf.ttfb}
                            unit=" ms"
                            statusColor={getPerformanceColor(perf.ttfb, 'ttfb')}
                        />
                        <MetricBox
                            label="FCP"
                            value={perf.fcp}
                            unit=" ms"
                            statusColor={getPerformanceColor(perf.fcp, 'fcp')}
                        />
                        <MetricBox
                            label="LCP"
                            value={perf.lcp}
                            unit=" ms"
                            statusColor={getPerformanceColor(perf.lcp, 'lcp')}
                        />
                        <MetricBox
                            label="DOM Load"
                            value={perf.domLoad}
                            unit=" ms"
                            statusColor={getPerformanceColor(perf.domLoad, 'dom')}
                        />
                    </Box>
                </MsqdxCard>
            )}
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                        ...
                        <Box
                            sx={{
                                ...
                                py: 1,
                                borderRadius: 1.5,
                                bgcolor: alpha(getEcoGradeColor(eco.grade), 0.08),
                                border: `1px solid ${alpha(getEcoGradeColor(eco.grade), 0.3)}`,
                            }}
                        >
                            ...
                            <MsqdxChip
                                ...
                                sx={{
                                    mt: 0.25,
                                    bgcolor: alpha(getEcoGradeColor(eco.grade), 0.2),
                                    color: getEcoGradeColor(eco.grade),
                                    fontWeight: 700,
                                }}
                            />
                        </Box>
            ...
            {seo && (
                ...
                >
                    <MsqdxTypography variant="subtitle1" sx={{ fontWeight: 600, mb: 1, color: textPrimary }}>
                        SEO
                    </MsqdxTypography>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
                        {seo.title && (
                            <Box>
                                ...
                                <MsqdxTypography variant="body2" sx={{ wordBreak: 'break-word', color: textPrimary }}>
                                    {seo.title.length > 80 ? seo.title.slice(0, 80) + '…' : seo.title}
                                </MsqdxTypography>
                            </Box>
                        )}
                        {seo.metaDescription && (
                            <Box>
                                ...
                                <MsqdxTypography variant="body2" sx={{ wordBreak: 'break-word', color: textPrimary }}>
                                    {seo.metaDescription.length > 120
                                        ? seo.metaDescription.slice(0, 120) + '…'
                                        : seo.metaDescription}
                                </MsqdxTypography>
                            </Box>
                        )}
                        {seo.h1 && (
                            <Box>
                                ...
                                <MsqdxTypography variant="body2" sx={{ wordBreak: 'break-word', color: textPrimary }}>
                                    {seo.h1.length > 80 ? seo.h1.slice(0, 80) + '…' : seo.h1}
                                </MsqdxTypography>
                            </Box>
                        )}
                        {!seo.title && !seo.metaDescription && !seo.h1 && (
            ...
            {links && (
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
                        ...
                        <MetricBox
                            label="Kaputte"
                            value={links.broken?.length ?? 0}
                            statusColor={(links.broken?.length ?? 0) > 0 ? STATUS.error : undefined}
                        />
                    </Box>
                </MsqdxCard>
            )}
                        ...
                        {privacy && (
                            <>
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={privacy.hasPrivacyPolicy ? 'Datenschutz' : 'Kein Datenschutz'}
                                    sx={
                                        privacy.hasPrivacyPolicy
                                            ? { bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }
                                            : { bgcolor: alpha(STATUS.error, 0.12), color: STATUS.error }
                                    }
                                />
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={privacy.hasCookieBanner ? 'Cookie-Hinweis' : 'Kein Cookie-Hinweis'}
                                    sx={
                                        privacy.hasCookieBanner
                                            ? { bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }
                                ...
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={privacy.hasTermsOfService ? 'AGB' : 'Keine AGB'}
                                    sx={
                                        privacy.hasTermsOfService
                                            ? { bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }
                                            : { bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }
                                    }
                                />
                            </>
                        )}
                        {security && (
                            <>
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={security.contentSecurityPolicy?.present ? 'CSP' : 'Kein CSP'}
                                    sx={
                                        security.contentSecurityPolicy?.present
                                            ? { bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }
                                            : { bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }
                                    }
                                />
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={security.xFrameOptions?.present ? 'X-Frame-Options' : 'Kein X-Frame-Options'}
                                    sx={
                                        security.xFrameOptions?.present
                                            ? { bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }
                                            : { bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }
                                    }
                                />
                ...
                >
                    ...
                    <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center' }}>
                        <MsqdxChip
                            size="small"
                            variant="outlined"
                            label={technicalInsights.manifest.present ? 'Web-App-Manifest' : 'Kein Manifest'}
                            sx={
                        ...
                        {technicalInsights.manifest.present && (
                            <>
                                {technicalInsights.manifest.hasName && (
                                    <MsqdxChip
                                        size="small"
                                        variant="outlined"
                                        label="Name"
                                        sx={{ bgcolor: alpha(STATUS.good, 0.1), color: STATUS.good }}
                                    />
                                )}
                                {technicalInsights.manifest.hasIcons && (
                                    <MsqdxChip
                                        size="small"
                                        variant="outlined"
                                        label="Icons"
                        ...
                        {technicalInsights.themeColor != null && technicalInsights.themeColor && (
                            <MsqdxChip
                                size="small"
                                variant="outlined"
                                label={`Theme ${technicalInsights.themeColor}`}
                                sx={{ bgcolor: alpha(STATUS.good, 0.1), color: STATUS.good }}
                            />
                        )}
                        ...
                        {technicalInsights.serviceWorkerRegistered && (
                            <MsqdxChip
                                ...
                                variant="outlined"
                                label="Service Worker"
                                sx={{ bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }}
                            />
                        )}
                        {technicalInsights.redirectCount != null && technicalInsights.redirectCount > 0 && (
                            <MsqdxChip
                                size="small"
                                variant="outlined"
                                label={`${technicalInsights.redirectCount} Redirect(s)`}
                                sx={{ bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }}
                            />
                        )}
                        ...
                        <Box sx={{ mb: geo ? 1.5 : 0 }}>
                            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center', mb: 0.5 }}>
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={`GEO-Score ${generative.score}`}
                                    sx={{
                                        bgcolor: alpha(getScoreColor(generative.score), 0.12),
                                        color: getScoreColor(generative.score),
                                        fontWeight: 600,
                                    }}
                                />
                                {generative.dimensions && (
                                    <>
                                        <MsqdxChip
                                            size="small"
                                            variant="outlined"
                                            label={`Auffindbarkeit ${generative.dimensions.discoverability}`}
                                            sx={{ bgcolor: alpha(STATUS.good, 0.08), fontWeight: 600 }}
                                        />
                                        <MsqdxChip
                                            size="small"
                                            variant="outlined"
                                            label={`Wiederverwertbarkeit ${generative.dimensions.repurposing}`}
                                            sx={{ bgcolor: alpha(STATUS.good, 0.08), fontWeight: 600 }}
                                        />
                                    </>
                                )}
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={generative.technical.hasLlmsTxt ? 'llms.txt' : 'Kein llms.txt'}
                                    sx={
                                        generative.technical.hasLlmsTxt
                                            ? { bgcolor: alpha(STATUS.good, 0.12), color: STATUS.good }
                                            : { bgcolor: alpha(STATUS.warning, 0.12), color: STATUS.warning }
                                    }
                                />
                                <MsqdxChip
                                    ...
                                    variant="outlined"
                                    label={
                                        generative.technical.hasRobotsAllowingAI
                                            ? 'Robots (AI)'
                                            : 'Robots (AI blockiert)'
                                    }
                                    sx={
                        ...
                        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center' }}>
                            {geo.location?.country && (
                                <MsqdxChip
                                    size="small"
                                    variant="outlined"
                                    label={[geo.location.city, geo.location.country].filter(Boolean).join(', ')}
                                />
                            )}
                            {geo.cdn.detected && geo.cdn.provider && (
                                <MsqdxChip size="small" variant="outlined" label={`CDN: ${geo.cdn.provider}`} />
                            )}
                ...
                >
                    ...
                    <Box component="ul" sx={{ m: 0, pl: 2, maxHeight: 300, overflow: 'auto' }}>
                        {dataIssues.slice(0, 50).map((issue, idx) => (
                            <li key={idx}>
                                <MsqdxTypography variant="body2" sx={{ color: textPrimary }}>
