...
export default function ProjectWcagPage() {
    ...
    const [competitorWcagScores, setCompetitorWcagScores] = useState<
        Record<string, { scanId: string; wcagScore: number; status: string }>
    >({});
    ...
    useEffect(() => {
        ...
        fetch(apiProject(id), { credentials: 'same-origin', signal })
            ...
            .catch(() => {
                if (!signal.aborted) setProject(null);
            })
            .finally(() => {
        ...
        Promise.all([
            ...
            .then(([domainSummaryRes, summaryAllRes]) => {
                ...
                if (summaryAllRes?.success && summaryAllRes?.data?.competitors) {
                    ...
                    for (const [domain, c] of Object.entries(
                        summaryAllRes.data.competitors as Record<
                            string,
                            { scanId: string; wcagScore?: number; status: string } | null
                        >,
                    )) {
                ...
                if (domainSummaryRes?.success && domainSummaryRes?.data?.scanId) {
                    ...
                    return fetch(apiScanDomainSummary(domainSummaryRes.data.scanId), {
            ...
            .catch((err) => {
                if (err?.name !== 'AbortError') setCompetitorWcagScores({});
            })
            .finally(() => {
                        ...
                        return (
                            <>
                                ...
                                >
                                    ...
                                    <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap', mb: 1 }}>
                                        <MsqdxChip
                                            ...
                                            variant="outlined"
                                            sx={{
                                                bgcolor: alpha(MSQDX_STATUS.error.base, 0.12),
                                ...
                                <MsqdxMoleculeCard
                                    ...
                                    sx={{ bgcolor: 'var(--color-card-bg)' }}
                                    headerActions={
                                        <InfoTooltip title={t('info.issuesList')} ariaLabel={t('common.info')} />
                                    }
                                >
                                    ...
                                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                                        ...
                                        >
                                            {aggregated.pagesByIssueCount.slice(0, 15).map((row) => {
                                                ...
                                                const issuesForPage = aggregated.issues.filter((i) =>
                                                    i.pageUrls?.includes(row.url),
                                                );
                                                return (
                                                    <PageIssuesCard
                                                        key={row.url}
                                                        ...
                                                        onOpenPage={
                                                            page ? () => router.push(pathResults(page.id)) : undefined
                                                        }
                                                    />
                                                );
                                            })}
