...
export default function ProjectGeoAnalysisPage() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                if (sid) {
                    const scanSummaryRes = await fetch(apiScanDomainSummary(sid), {
                ...
                const latestRes = (await fetch(apiProjectGeoLatestResult(id), {
                    credentials: 'same-origin',
                    signal,
                }).then((r) => r.json())) as {
                    success?: boolean;
                    data?: { payload?: { recommendations?: GeoEeatRecommendation[] } } | null;
                };
    ...
    const timestamp = fullSummary?.timestamp
        ? new Date(fullSummary.timestamp).toLocaleString(undefined, { dateStyle: 'medium', timeStyle: 'short' })
        : '';
                                                ...
                                                summary={
                                                    ...
                                                    >
                                                        ...
                                                        {rec.dimension && (
                                                            <MsqdxChip
                                                                label={t(
                                                                    `projects.geoAnalysis.dimension${rec.dimension.charAt(0).toUpperCase()}${rec.dimension.slice(1)}`,
                                                                )}
                                                                size="small"
                                                                variant="outlined"
                                                                sx={{ fontSize: '0.65rem' }}
