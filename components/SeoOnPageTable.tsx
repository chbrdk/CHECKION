        ...
        >
            <Box sx={{ px: 1, py: 0.5, minWidth: 0, display: 'flex', alignItems: 'center', borderRight: tableBorder }}>
                {onRowClick ? (
                    ...
                    <MsqdxTypography
                        variant="caption"
                        sx={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontSize: '0.75rem' }}
                    >
                        {row.url}
                    </MsqdxTypography>
                )}
                ...
                {row.isSkinny ? (
                    <MsqdxChip
                        ...
                        sx={{
                            height: 18,
                            fontSize: '0.65rem',
                            bgcolor: alpha(MSQDX_STATUS.warning.base, 0.15),
                    ...
                    <MsqdxTypography
                        variant="caption"
                        sx={{ color: MSQDX_THEME.light.text.tertiary, fontSize: '0.7rem' }}
                    >
                        –
                    </MsqdxTypography>
                )}
            ...
            <Box sx={{ px: 0.5, py: 0.5, display: 'flex', alignItems: 'center', borderRight: tableBorder }}>
                <MsqdxTypography variant="caption" sx={{ fontSize: '0.7rem', fontWeight: 500 }}>
                    {row.hasMeta ? tableYes : tableNo}
                </MsqdxTypography>
            </Box>
            <Box sx={{ px: 0.5, py: 0.5, display: 'flex', alignItems: 'center', borderRight: tableBorder }}>
                <MsqdxTypography variant="caption" sx={{ fontSize: '0.7rem', fontWeight: 500 }}>
                    {row.hasH1 ? tableYes : tableNo}
                </MsqdxTypography>
            </Box>
                ...
                {row.structure === 'good' ? (
                    <MsqdxChip
                        ...
                        sx={{
                            height: 18,
                            fontSize: '0.65rem',
                            bgcolor: alpha(MSQDX_BRAND_PRIMARY.green, 0.15),
...
function SeoOnPageTableInner({
    ...
    const paginatedRows = useMemo(() => sortedRows.slice(start, start + pageSize), [sortedRows, start, pageSize]);
    ...
    const visibleRows = useMemo(() => paginatedRows.slice(startIndex, endIndex), [paginatedRows, startIndex, endIndex]);
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <SortHeaderCell
                            id="wordCount"
                            label={t('projects.seo.tableWords')}
                        ...
                        <SortHeaderCell
                            id="topKeywordCount"
                            label={t('projects.seo.tableKeywords')}
                            active={sortKey === 'topKeywordCount'}
                            sortDir={sortDir}
                            onSort={handleSort}
                        />
                        <SortHeaderCell
                            id="isSkinny"
                            label={t('projects.seo.tableSkinny')}
                            active={sortKey === 'isSkinny'}
                            sortDir={sortDir}
                            onSort={handleSort}
                        />
                        <SortHeaderCell
                            id="hasMeta"
                            label={t('projects.seo.tableMeta')}
                            active={sortKey === 'hasMeta'}
                            sortDir={sortDir}
                            onSort={handleSort}
                        />
                        <SortHeaderCell
                            id="hasH1"
                            label={t('projects.seo.tableH1')}
                            active={sortKey === 'hasH1'}
                            sortDir={sortDir}
                            onSort={handleSort}
                        />
                        <SortHeaderCell
                            id="structure"
                            label={t('projects.seo.tableStructure')}
                            active={sortKey === 'structure'}
                            sortDir={sortDir}
                            onSort={handleSort}
                        />
                    </Box>
                    {startIndex > 0 && (
                        <Box
                            component="div"
                            role="presentation"
                            sx={{ height: startIndex * ROW_HEIGHT, flexShrink: 0 }}
                        />
                    )}
                    {visibleRows.map((row) => (
                        <TableRow
                    ...
                    {endIndex < paginatedRows.length && (
                        <Box
                            component="div"
                            role="presentation"
                            sx={{ height: (paginatedRows.length - endIndex) * ROW_HEIGHT, flexShrink: 0 }}
                        />
                    )}
                </Box>
            </Box>
