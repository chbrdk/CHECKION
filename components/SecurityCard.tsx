            ...
            <Box sx={{ display: 'grid', gap: 'var(--msqdx-spacing-xs)' }}>
                    ...
                    return (
                        ...
                        >
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 'var(--msqdx-spacing-sm)' }}>
                                <Box
                                    sx={{
                                        color: present ? MSQDX_BRAND_PRIMARY.green : MSQDX_STATUS.error.base,
                                        display: 'flex',
                                    }}
                                >
                            ...
                            {present ? (
                                <MsqdxChip
                                    ...
                                    variant="outlined"
                                    sx={{
                                        bgcolor: alpha(MSQDX_STATUS.success.base, 0.1),
                ...
                {security.mixedContentUrls && security.mixedContentUrls.length > 0 && (
                    ...
                    >
                        <MsqdxTypography
                            variant="subtitle2"
                            sx={{ color: MSQDX_STATUS.error.base, fontWeight: 600, mb: 'var(--msqdx-spacing-xxs)' }}
                        >
                ...
                {security.sriMissing && security.sriMissing.length > 0 && (
                    ...
                    >
                        <MsqdxTypography
                            variant="subtitle2"
                            sx={{ color: MSQDX_STATUS.warning.base, fontWeight: 600, mb: 'var(--msqdx-spacing-xxs)' }}
                        >
                            Externe Ressourcen ohne SRI (integrity)
                        </MsqdxTypography>
                        {security.sriMissing.slice(0, 5).map((s, i) => (
                            <MsqdxTypography
                                key={i}
                                variant="caption"
                                sx={{ display: 'block', color: 'var(--color-text-on-light)', wordBreak: 'break-all' }}
                            >
                                {s.tag}: {s.url}
                            </MsqdxTypography>
                        ))}
                ...
                {security.cookieWarnings && security.cookieWarnings.length > 0 && (
                    ...
                    >
                        ...
                        {security.cookieWarnings.map((w, i) => (
                            <MsqdxTypography key={i} variant="body2" sx={{ color: 'var(--color-text-on-light)' }}>
                                {w.message}
                            </MsqdxTypography>
                        ))}
                    </Box>
