import { Box, alpha } from '@mui/material';
import {
    MsqdxTypography,
    MsqdxMoleculeCard,
    MsqdxChip,
    MsqdxAccordion,
    MsqdxAccordionItem,
} from '@msqdx/react';
import { MSQDX_SPACING, MSQDX_THEME, MSQDX_STATUS, MSQDX_BRAND_PRIMARY, MSQDX_NEUTRAL } from '@msqdx/tokens';
import type { LinkAudit } from '@/lib/types';
import { Link2, Link, ExternalLink, AlertTriangle, CheckCircle } from 'lucide-react';

export function LinkAuditCard({ links }: { links: LinkAudit }) {
    if (!links) return null;

    return (
        <MsqdxMoleculeCard
            title="Link Audit"
            subtitle="Analysis of hyperlinks and their availability."
            variant="flat"
            borderRadius="lg"
            sx={{ height: '100%' }}
        >
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: MSQDX_SPACING.scale.md, mb: MSQDX_SPACING.scale.lg }}>
                <StatItem label="Total Links" value={links.total} icon={<Link2 size={16} />} />
                <StatItem label="Internal" value={links.internal} icon={<Link size={16} />} />
                <StatItem label="External" value={links.external} icon={<ExternalLink size={16} />} />
            </Box>

            {links.broken.length > 0 ? (
                <Box>
                    <MsqdxTypography variant="subtitle2" sx={{ mb: MSQDX_SPACING.scale.sm, color: MSQDX_STATUS.error.base, display: 'flex', alignItems: 'center', gap: 1 }}>
                        <AlertTriangle size={16} />
                        Broken Links ({links.broken.length})
                    </MsqdxTypography>
                    <Box sx={{
                        borderRadius: MSQDX_SPACING.borderRadius.md,
                        border: `1px solid ${MSQDX_STATUS.error.border}`,
                        bgcolor: alpha(MSQDX_STATUS.error.base, 0.05),
                        overflow: 'hidden'
                    }}>
                        {links.broken.map((link, i) => (
                            <Box key={i} sx={{
                                p: MSQDX_SPACING.scale.sm,
                                borderBottom: i < links.broken.length - 1 ? `1px solid ${alpha(MSQDX_STATUS.error.border, 0.5)}` : 'none',
                                display: 'flex',
                                justifyContent: 'space-between',
                                alignItems: 'center'
                            }}>
                                <Box sx={{ overflow: 'hidden' }}>
                                    <MsqdxTypography variant="body2" sx={{ fontWeight: 600, color: MSQDX_THEME.dark.text.primary }}>
                                        {link.text || 'No Text'}
                                    </MsqdxTypography>
                                    <MsqdxTypography variant="caption" sx={{ color: MSQDX_THEME.dark.text.secondary, display: 'block', maxWidth: '300px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        {link.url}
                                    </MsqdxTypography>
                                </Box>
                                <MsqdxChip
                                    label={link.statusCode || 'Error'}
                                    size="small"
                                    sx={{
                                        bgcolor: MSQDX_STATUS.error.base,
                                        color: '#fff',
                                        fontWeight: 'bold',
                                        height: 24
                                    }}
                                />
                            </Box>
                        ))}
                    </Box>
                </Box>
            ) : (
                <Box sx={{
                    p: MSQDX_SPACING.scale.md,
                    bgcolor: alpha(MSQDX_STATUS.success.base, 0.05),
                    borderRadius: MSQDX_SPACING.borderRadius.md,
                    display: 'flex',
                    alignItems: 'center',
                    gap: 2
                }}>
                    <CheckCircle size={24} color={MSQDX_STATUS.success.base} />
                    <Box>
                        <MsqdxTypography variant="subtitle2" sx={{ color: MSQDX_STATUS.success.dark }}>
                            No broken links found
                        </MsqdxTypography>
                        <MsqdxTypography variant="caption" sx={{ color: MSQDX_THEME.dark.text.secondary }}>
                            Verified first 25 links.
                        </MsqdxTypography>
                    </Box>
                </Box>
            )}
        </MsqdxMoleculeCard>
    );
}

function StatItem({ label, value, icon }: { label: string, value: number, icon: React.ReactNode }) {
    return (
        <Box sx={{ textAlign: 'center', p: 1, bgcolor: alpha(MSQDX_NEUTRAL[200], 0.05), borderRadius: '8px' }}>
            <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 1, mb: 0.5, color: MSQDX_THEME.dark.text.tertiary }}>
                {icon}
                <MsqdxTypography variant="caption">{label}</MsqdxTypography>
            </Box>
            <MsqdxTypography variant="h6" sx={{ fontWeight: 700 }}>{value}</MsqdxTypography>
        </Box>
    );
}
