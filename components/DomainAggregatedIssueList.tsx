...
function DomainAggregatedIssueListInner({
    ...
    const sortedIssues = useMemo(() => {
        ...
        arr.sort((a, b) => {
            ...
            switch (sortKey) {
                case 'severity':
                    cmp =
                        (severityOrder[a.type as keyof typeof severityOrder] ?? 2) -
                        (severityOrder[b.type as keyof typeof severityOrder] ?? 2);
                    break;
                case 'message':
    ...
    const paginatedRows = useMemo(() => sortedIssues.slice(start, start + pageSize), [sortedIssues, start, pageSize]);
            ...
            >
                ...
                >
                    ...
                    >
                        <SortHeader
                            id="severity"
                            label={t('domainResult.issuesTableSeverity')}
                            active={sortKey === 'severity'}
                        />
                        <SortHeader
                            id="message"
                            label={t('domainResult.issuesTableMessage')}
                            active={sortKey === 'message'}
                        />
                        <SortHeader
                            id="level"
                            label={t('domainResult.issuesTableLevel')}
                            active={sortKey === 'level'}
                        />
                        <SortHeader
                            id="runner"
                            label={t('domainResult.issuesTableRunner')}
                            active={sortKey === 'runner'}
                        />
                        <SortHeader id="code" label={t('domainResult.issuesTableCode')} active={sortKey === 'code'} />
                        <SortHeader
                            id="pageCount"
                            label={t('domainResult.issuesTablePages')}
                            active={sortKey === 'pageCount'}
                        />
