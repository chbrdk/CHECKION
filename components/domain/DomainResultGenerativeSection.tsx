...
function DomainResultGenerativeSectionInner({
    ...
    return (
        <Profiler id="TEST_1" onRender={(...args) => console.log('Profiler TEST_1:', args)}>
            <MsqdxMoleculeCard
            ...
            >
                <Stack spacing={2} sx={{ width: '100%', minWidth: 0 }}>
                    ...
                    >
                        {kpiCell(
                            'domainResult.generativeKpiGeoScore',
                            String(generative.score),
                            geoScoreColor(generative.score),
                        )}
                        {kpiCell(
                            'domainResult.generativeKpiDiscoverability',
                            generative.avgDiscoverability != null ? String(generative.avgDiscoverability) : '—',
                            generative.avgDiscoverability != null
                                ? geoScoreColor(generative.avgDiscoverability)
                                : undefined,
                        )}
                        {kpiCell(
...
function DomainResultGenerativeEmptyInner({
    t,
}: {
    t: (key: string, params?: Record<string, string | number>) => string;
}) {
