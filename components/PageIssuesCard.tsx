...
export function PageIssuesCard({
    ...
    const sorted = [...issuesForPage].sort(
        (a, b) =>
            (SEVERITY_ORDER[a.type as keyof typeof SEVERITY_ORDER] ?? 2) -
            (SEVERITY_ORDER[b.type as keyof typeof SEVERITY_ORDER] ?? 2),
    );
            ...
            <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.5, mb: 1.5 }}>
                {stats.errors > 0 && (
                    <MsqdxChip
                        ...
                        sx={{
                            bgcolor: alpha(SEVERITY_CONFIG.error.color, 0.12),
                            color: SEVERITY_CONFIG.error.color,
                            fontSize: '0.7rem',
                        }}
                    />
                )}
                {stats.warnings > 0 && (
                    <MsqdxChip
                        ...
                        sx={{
                            bgcolor: alpha(SEVERITY_CONFIG.warning.color, 0.12),
                ...
                {stats.notices > 0 && (
                    <MsqdxChip
                        ...
                        sx={{
                            bgcolor: alpha(SEVERITY_CONFIG.notice.color, 0.12),
                            color: SEVERITY_CONFIG.notice.color,
                            fontSize: '0.7rem',
                        }}
                    />
                )}
