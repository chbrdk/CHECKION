        ...
        <React.Fragment key="issues">
            ...
            {scan.issues.length === 0 ? (
                ...
                scan.issues.slice(0, 25).map((issue, idx) => (
                    <View key={idx} style={[styles.issueRow, { borderLeftColor: SeverityColor[issue.type] ?? colors.gray400 }]}>
            ...
            {scan.issues.length > 25 && (
                <Text style={[styles.metaText, { marginTop: 8, fontStyle: 'italic' }]}>
            ...
            <React.Fragment key="summary">
                ...
                {scan.llmSummary.overallGrade && (
                    <View style={styles.cardBox}>
                        <Text style={styles.bodyText}>Gesamtnote: {scan.llmSummary.overallGrade}</Text>
                    </View>
                )}
        ...
        allContent.push(
            <React.Fragment key="ux">
                ...
                <View style={styles.cardBox}>
                    ...
                    <View style={styles.tableRow}>
                        <Text style={styles.tableLabel}>Console-Fehler / Kaputte Links</Text>
                        <Text style={styles.tableValue}>{ux.consoleErrors?.length ?? 0} / {ux.brokenLinks?.length ?? 0}</Text>
                    </View>
                    {ux.hasSkipLink != null && (
                        <View style={styles.tableRow}>
            ...
            <React.Fragment key="seo">
                ...
                {scan.seo && (
                    <View style={styles.cardBox}>
                        ...
                        <View style={styles.tableRow}>
                            <Text style={styles.tableLabel}>H1 / Canonical / OG</Text>
                            <Text style={styles.tableValue}>
                                {scan.seo.h1 ?? '–'} / {scan.seo.canonical ? 'vorhanden' : '–'} / {scan.seo.ogTitle ? 'vorhanden' : '–'}
                            </Text>
                        </View>
                ...
                {scan.links && (
                    <>
                        ...
                        <View style={styles.tableRow}>
                            <Text style={styles.tableLabel}>Gesamt / Intern / Extern / Kaputt</Text>
                            <Text style={styles.tableValue}>
                                {scan.links.total} / {scan.links.internal} / {scan.links.external} / {scan.links.broken?.length ?? 0}
                            </Text>
                        </View>
                        {scan.links.broken && scan.links.broken.length > 0 && scan.links.broken.slice(0, 5).map((b, i) => (
                            <Text key={i} style={styles.bodyText}>{b.url} → {b.statusCode}</Text>
                        ))}
                    </>
                )}
            </React.Fragment>
            ...
            <React.Fragment key="infra">
                ...
                <View style={styles.cardBox}>
                    {scan.geo && (
                        <>
                            <View style={styles.tableRow}>
                                <Text style={styles.tableLabel}>Server / CDN / Standort</Text>
                                <Text style={styles.tableValue}>
                                    {scan.geo.serverIp ?? '–'} / {scan.geo.cdn?.provider ?? '–'} / {scan.geo.location?.city ?? '–'}, {scan.geo.location?.country ?? '–'}
                                </Text>
                            </View>
                            <View style={styles.tableRow}>
                                <Text style={styles.tableLabel}>html lang / hreflang</Text>
                                <Text style={styles.tableValue}>
                                    {scan.geo.languages?.htmlLang ?? '–'} / {scan.geo.languages?.hreflangs?.length ?? 0}
                                </Text>
                            </View>
                        </>
                    )}
                    {scan.privacy && (
                        <View style={styles.tableRow}>
                            ...
                            <Text style={styles.tableValue}>
                                {scan.privacy.hasPrivacyPolicy ? 'Ja' : 'Nein'} / {scan.privacy.hasCookieBanner ? 'Ja' : 'Nein'} / {scan.privacy.hasTermsOfService ? 'Ja' : 'Nein'}
                            </Text>
                        </View>
                    )}
                    {scan.security && (
                        <View style={styles.tableRow}>
                            ...
                            <Text style={styles.tableValue}>
                                {scan.security.contentSecurityPolicy?.present ? 'Ja' : 'Nein'} / {scan.security.xFrameOptions?.present ? 'Ja' : 'Nein'} / {scan.security.strictTransportSecurity?.present ? 'Ja' : 'Nein'}
                            </Text>
                        </View>
                    )}
                    {scan.technicalInsights && (
                        <View style={styles.tableRow}>
                            <Text style={styles.tableLabel}>Manifest / Theme-Color / Apple Touch</Text>
                            <Text style={styles.tableValue}>
                                {scan.technicalInsights.manifest?.present ? 'Ja' : 'Nein'} / {scan.technicalInsights.themeColor ?? '–'} / {scan.technicalInsights.appleTouchIcon ? 'Ja' : 'Nein'}
                            </Text>
                        </View>
                    )}
                </View>
        ...
        allContent.push(
            <React.Fragment key="generative">
                ...
                <View style={styles.cardBox}>
                    ...
                    <View style={styles.tableRow}>
                        <Text style={styles.tableLabel}>FAQ / Tabellen / Autor / Zitate</Text>
                        <Text style={styles.tableValue}>
                            {g.content?.faqCount ?? 0} / {g.content?.tableCount ?? 0} / {g.expertise?.hasAuthorBio ? 'Ja' : 'Nein'} / {g.expertise?.hasExpertCitations ? 'Ja' : 'Nein'}
                        </Text>
                    </View>
                </View>
            </React.Fragment>
