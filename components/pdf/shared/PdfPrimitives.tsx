...
export function MsqdxLogoPdf({ width, height }: { width: number; height: number }) {
    return (
        <Svg viewBox="0 0 167 40" width={width} height={height}>
            <Path d={MSQDX_LOGO_PATH_D} fill={pdfColors.black} />
        </Svg>
    );
}
...
export function RiskAmpelPills({
    ampel,
    labels,
}: {
    ampel: { wcag: string; geo: string; rankings: string };
    labels: { wcag: string; geo: string; rankings: string };
}) {
    const colorFor = (level: string) => {
        if (level === 'high') return pdfColors.error;
        if (level === 'medium') return pdfColors.warning;
        if (level === 'low') return pdfColors.success;
        return pdfColors.gray400;
    };
    const items = [
        { key: labels.wcag, level: ampel.wcag },
        { key: labels.geo, level: ampel.geo },
        { key: labels.rankings, level: ampel.rankings },
    ];
    return (
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', marginBottom: 8 }}>
            {items.map((item) => (
                <View
                    key={item.key}
                    style={[pdfStyles.riskPill, { backgroundColor: colorFor(item.level) + '22' }]}
                >
                    <Text style={{ fontSize: 8, color: pdfColors.gray700 }}>
                        {item.key}: {item.level.toUpperCase()}
                    </Text>
                </View>
            ))}
        </View>
