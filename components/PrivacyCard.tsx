    ...
    return (
        ...
        >
            <Box sx={{ display: 'grid', gap: 'var(--msqdx-spacing-xs)' }}>
                ...
                />
                <PrivacyItem label="Cookie Consent" present={privacy.hasCookieBanner} icon={<Cookie size={18} />} />
                <PrivacyItem
                ...
                {consentSignals && (
                    <Box
                        sx={{
                            ...
                            borderRadius: 1,
                            bgcolor: alpha(MSQDX_BRAND_PRIMARY.purple, 0.06),
                            border: `1px solid ${alpha(MSQDX_BRAND_PRIMARY.purple, 0.2)}`,
                        }}
                    >
                        ...
                        {consentSignals.tcfApiPresent && (
                            <MsqdxTypography
                                variant="caption"
                                sx={{ display: 'block', color: 'var(--color-text-on-light)' }}
                            >
                                TCF: __tcfapi erkannt
                            </MsqdxTypography>
                        )}
                        ...
                        {(consentSignals.earlyThirdPartyScriptHosts?.length ?? 0) > 0 && (
                            ...
                            >
                                Frühe Drittanbieter-Skript-Hosts:{' '}
                                {consentSignals.earlyThirdPartyScriptHosts!.slice(0, 12).join(', ')}
                ...
                <Box
                    sx={{
                        ...
                        p: 'var(--msqdx-spacing-sm)',
                        borderRadius: 1,
                        bgcolor: alpha(MSQDX_STATUS.info.base, 0.05),
                        border: `1px dashed ${alpha(MSQDX_STATUS.info.base, 0.3)}`,
                    }}
                >
                    <MsqdxTypography
                        variant="caption"
    ...
    return (
        ...
        >
            <Box sx={{ display: 'flex', alignItems: 'center', gap: 'var(--msqdx-spacing-sm)' }}>
                ...
                <Box>
                    <MsqdxTypography variant="subtitle2" sx={{ fontWeight: 600, fontSize: '0.85rem' }}>
                        {label}
                    </MsqdxTypography>
                    {url && (
                        <MsqdxTypography
